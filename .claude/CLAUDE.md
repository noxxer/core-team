...
- **Проверщик рефлексии** (`hooks/check-session-reflection.sh`) — не хук: вызывается из `/end-session` (Шаг 7). Проверяет, что в записанной сессии заполнены слоты раздела «Смыслы» (Iceberg / Три «Почему» / System Operator / Ловушки). Замер: анализ есть в 68–75% сессий, а в отчёт Founder-у попадает в 27%. Доказательство мутацией — `check-session-reflection.test.sh` (14 случаев).
