...
- **Проверщик решений** (`hooks/check-decisions.sh`) — не хук: вызывается из `/end-session` (Шаг 6a). Проверяет, что у каждого решения после даты внедрения заполнено `enforced_by` — адрес проверки, делающей нарушение красным. Унаследованные решения перечисляются отдельно и заполняются при первом касании. Доказательство мутацией — `hooks/check-decisions.test.sh` (14 случаев), прогон — CI `guards`.
