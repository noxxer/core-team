...
(фокус, устойчивое знание, открытые вопросы, решения, урок ремесла), а не запись в ленту.
...
- **SessionStart** (`hooks/session-start.sh`) — один из **трёх** хуков, привязанных к событию (другие два — `SubagentStop` и `PreToolUse`; рядом в `hooks/` лежат проверщики, вызываемые командами, — см. ниже); событие с подтверждённой частотой срабатывания. Делает четыре вещи. (1) Инжектит протокол: Facilitator-default, 3-стадийный вопрос, fail-fast, Code-Change Discipline, FPF-гейты (NQD/A.7/A.10/A.1.1/A.11), стартовый ритуал чтения артефактов. **Активирует FPF "по умолчанию" в каждой сессии** — без хука FPF не используется автоматически. (2) **Лестница смыслов:** печатает из `ledger.md` три уровня — миссия → цель фазы → ближайший шаг, и помечает поле, разросшееся в хронику (>200 символов). Замер: в боевом проекте поле фазы за 172 сессии превратилось в пересказ последней сессии. (3) **Термометр памяти ролей:** печатает роли, чья `context.md` отстаёт больше `MEMORY_STALE_DAYS` (по умолчанию 7) или пуста. Замер на двух проектах: у 10 ролей из 22 память старше их последней работы, и увидеть это было негде. (4) **Объём памяти ролей:** делегирует `check-role-memory.sh` и печатает находки, ничего не блокируя. Приборы намеренно разные: возраст и объём мерят разное, и на боевых данных термометр возраста был зелёным ровно на трёх ролях, чья память физически не открывается. Доказательство мутацией — `hooks/session-start.test.sh`, прогон — CI `guards`.
...
- **Целостность копии** (`hooks/check-install-integrity.sh`) — не хук: прогоняется в CI `guards`, в аудите `/self-service` и в Шаге 9 `/setup-project`. Проверяет, что развёрнутая копия **работоспособна**, а не просто состоит из файлов: `.claude/VERSION` на месте и непуст · все `hooks/*.sh` исполняемы · каждый объявленный в `settings.json` хук существует **внутри этой копии** (путь ищется только под её корнем — запасной поиск по текущему каталогу давал ложно-зелёное) · хоть один хук объявлен · **ремесло роли доехало** (каждая роль называет свой `knowledge/dpf/<ремесло>.md`, файл существует, слой DPF не пуст) · правила `rules/*.md` ведут в существующие файлы · слоты шаблона сессии совпадают со списком в проверщике рефлексии · числа случаев, заявленные в этом файле, совпадают с `EXPECTED_CASES` наборов. Половина «найдено не ноль»: пустой перечень хуков — отказ, а не успех. Доказательство мутацией — `check-install-integrity.test.sh` (29 случаев, десять мутаций).
