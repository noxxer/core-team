...
- **Сборщик кандидатов наверх** (`hooks/collect-upstream-candidates.sh`) — не хук: вызывается из `/end-session` (Шаг 6b). Находит маркеры `upstream-candidate` в `.claude/**`, проверяет корзину по белому списку и наличие причины. Доказательство мутацией — `collect-upstream-candidates.test.sh` (10 случаев), прогон — CI `guards`.
