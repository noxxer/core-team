...
9. **Память роли — состояние, а не журнал.** `project/roles/<role>/context.md` отвечает на «что я знаю и где я сейчас»; хроника по дням живёт в `project/sessions/`. Дописать строку дешевле, чем переписать раздел, поэтому лента побеждает состояние, если её пустить, — а у ленты нет дна. Потолок: 200 строк (`max_lines:` в шапке поднимает осознанно) и жёсткий предел чтения инструмента **256 КБ**. Проверяет `hooks/check-role-memory.sh` — в `/end-session` (Шаг 5b), в SessionStart, в аудите `/self-service` и **обязательно при миграции состояния** в `/setup-project`. Замер: у трёх самых нагруженных ролей боевого проекта память доросла до 571.5 / 334.5 / 303.5 КБ — `Read` отказывает, ритуал «прочитай свою память» исполняется вхолостую, и агент не падает, а молча работает без памяти. Термометр возраста при этом печатал «в норме»: он мерил свежесть и был слеп к объёму.
