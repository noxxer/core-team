...
И **на выходе** возвращает последним разделом отчёта блок «Для памяти роли» (дата, что сделано,
решения, следующий шаг). Facilitator приземляет его в `project/roles/<role>/context.md`, если
записи за этот день там ещё нет. Память роли — побочный продукт конвейера, а не отдельный долг:
роль формулирует, оркестратор записывает, термометр SessionStart показывает, где это не сработало.
...
> старое не править». `.claude/knowledge/**` в запрет не входит намеренно — там генерируется
...
- **SessionStart** (`hooks/session-start.sh`) — один из **двух** хуков, привязанных к событию (второй — `SubagentStop`; рядом в `hooks/` лежат проверщики, вызываемые командами, — см. ниже); событие с подтверждённой частотой срабатывания. Делает три вещи. (1) Инжектит протокол: Facilitator-default, 3-стадийный вопрос, fail-fast, Code-Change Discipline, FPF-гейты (NQD/A.7/A.10/A.1.1/A.11), стартовый ритуал чтения артефактов. **Активирует FPF "по умолчанию" в каждой сессии** — без хука FPF не используется автоматически. (2) **Лестница смыслов:** печатает из `ledger.md` три уровня — миссия → цель фазы → ближайший шаг, и помечает поле, разросшееся в хронику (>200 символов). Замер: в боевом проекте поле фазы за 172 сессии превратилось в пересказ последней сессии. (3) **Термометр памяти ролей:** печатает роли, чья `context.md` отстаёт больше `MEMORY_STALE_DAYS` (по умолчанию 7) или пуста. Замер на двух проектах: у 10 ролей из 22 память старше их последней работы, и увидеть это было негде. Доказательство мутацией — `hooks/session-start.test.sh`, прогон — CI `guards`.
