...
- **SessionStart** (`hooks/session-start.sh`) — **единственный хук, привязанный к событию** (рядом в `hooks/` лежат проверщики, вызываемые командами, — см. ниже); событие с подтверждённой частотой срабатывания. Делает две вещи. (1) Инжектит протокол: Facilitator-default, 3-стадийный вопрос, fail-fast, Code-Change Discipline, FPF-гейты (NQD/A.7/A.10/A.1.1/A.11), стартовый ритуал чтения артефактов. **Активирует FPF "по умолчанию" в каждой сессии** — без хука FPF не используется автоматически. (2) **Термометр памяти ролей:** печатает роли, чья `context.md` отстаёт больше `MEMORY_STALE_DAYS` (по умолчанию 7) или пуста. Замер на двух проектах: у 10 ролей из 22 память старше их последней работы, и увидеть это было негде. Доказательство мутацией — `hooks/session-start.test.sh`, прогон — CI `guards`.
