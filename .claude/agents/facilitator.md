...
- **Приземление памяти роли — после каждого `Agent(subagent_type=<role>)`, без исключений:**
  ...
  2. **Переписать** одноимённые разделы `project/roles/<role>/context.md` — фокус, знания,
     открытые вопросы, решения, уроки — и обновить `last_updated`. Именно переписать, а не
     дописать датированный блок сверху: `context.md` — состояние роли, а не лента. Хроника дня
     живёт в `project/sessions/`. Закрывшийся вопрос из «открытых» **убирается**, а не
     помечается закрытым — след остаётся в сессиях и решениях.
  ...
  **Потолок памяти механизирован.** `hooks/check-role-memory.sh` краснеет, когда память роли
  переросла предел чтения (256 КБ), вышла за бюджет строк или снова поехала лентой; SessionStart
  печатает это в начале сессии, `/end-session` не закрывается по-тихому. Замер, ради которого:
