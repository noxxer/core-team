...
- **Приземление памяти роли — после каждого `Agent(subagent_type=<role>)`, без исключений:**
  ...
  2. Если в `project/roles/<role>/context.md` нет записи за сегодняшнюю дату — дописать блок
     и обновить `last_updated`. Есть — ничего не дублировать.
  3. Раздела в отчёте нет — это **дефект брифа, а не мелочь**: записать в память роли строку
     «<дата> — роль не вернула блок памяти по задаче <какой>» и назвать это в ответе Founder.
     Молча пропустить нельзя: невидимый пропуск и есть механизм, которым память протухает.

  Это заменило хук `TaskCompleted`, который за 143 закрытия задач не сработал ни разу.
  Проверка та же, но живёт там, где исполняется гарантированно — в цикле диспатча.
