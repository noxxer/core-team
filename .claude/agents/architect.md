...
Алгоритм — по канону: `.claude/knowledge/code-change-discipline.md`. Состав шагов здесь **не
пересказан намеренно**: пересказ жил в этом файле и в `dev.md` слово в слово, называл себя
«7 шагов» и перечислял шесть — терялось звено «поставить идею под сомнение первой», то есть
ровно то, что для правки архитектуры дороже всего.

Запрет из канона, который твоя работа задевает прежде прочих: **контракт метода/API нельзя
менять без явного обсуждения** — это breaking change.
