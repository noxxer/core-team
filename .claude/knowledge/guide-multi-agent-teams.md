...
файл `hooks.json` в каталоге хуков Claude Code **не читает**: этот путь принадлежит плагинам
...
- [ ] Объявить hooks в `.claude/settings.json` (НЕ в отдельном файле `hooks.json` внутри каталога хуков — его не читают)
