...
| Гард | Guard | Проверка-сторож: механически ловит класс дефектов и падает при его появлении (тест, хук, правило CI) | Тест новой фичи — тот описывает поведение; гард защищает от возврата известного класса ошибок |
| Ложно-зелёная проверка | Vacuous Check | Проверка, зелёная не потому что дефекта нет, а потому что она его не смотрит: перечисление пусто, вход не тот, падение никто не потребляет. В уроках проектов — «вакуозный гард» | Флейки — флейк мигает, ложно-зелёная стабильно зелена и потому незаметна |
| Мутационная проба | Mutation Proof | Единичный акт приёмки гарда: намеренно вернуть дефект и показать, что проверка краснеет | Мутационное тестирование (score) — то метрика сьюта по кодовой базе, проба — приёмка одного гарда |
| Половина «найдено не ноль» | Non-Empty Assertion | Вторая половина перечисляющего гарда: утверждение, что перечисление непусто (просмотрено ≥ N объектов) | Проверка количества нарушений — эта половина проверяет, что вообще было что просматривать |
...
- **Последствия:** Снижает прод-инциденты от schema mismatch. Выявляет устаревшие заглушки. Создаёт давление на команду поддерживать живые контракты.
...
### P-13: Mutation Proof — приёмка гарда через предъявленную красноту

- **Проблемная ситуация:** Гард добавлен после починки бага и зелёный. Зелёный он потому, что дефекта больше нет, — или потому, что не различает мир с дефектом и мир без него? Снаружи оба состояния выглядят одинаково.
- **Силы:** Тест пишется сразу после кода, тем же автором, в тех же терминах — и наследует его слепые зоны. Название теста отражает намерение автора, а не то, что тест механически проверяет. В момент написания гарда дефекта уже нет, поэтому краснота ни разу не наблюдалась. Мутация, выбранная автором, систематически проверяет то, о чём автор уже подумал.
- **Ход (решение):** Гард не считается принятым, пока не предъявлено: (1) **какая мутация** возвращает дефект; (2) **подтверждение дифом**, что мутация попала туда, куда заявлено; (3) мутация **в той форме, в какой дефект встречается в коде**; (4) **вывод красного прогона до и зелёного после**. Выжившая мутация читается как «тест проходит другим путём» — путь называется, отрезается в сценарии, мутация повторяется. Ревьюер выбирает мутации, которых **не пробовал автор**.
- **Последствия:** Разделяет «зелёный» и «поймает регресс» — до сих пор это различали только постфактум, по пропущенным в прод дефектам. Даёт ревью проверяемый предмет вместо суждения о качестве теста.
- **Trade-offs:** Минуты на гард. Требует дисциплины при правке чужого кода: мутация «первого вхождения» умеет попасть в соседний файл, и гард честно молчит. Не применимо к контурам, которые нельзя безопасно мутировать.
- **Пример:** Аудит собственных приборов в одном проекте нашёл **9 гардов**, не различавших мир с дефектом и без. В другом ревьюер снял защиту двумя мутациями, которых автор не пробовал: покраснело **0 тестов из ~44** — тесты мокали композабл фиксированным значением и проверяли, что шаблон его рендерит, а не что значение доходит до резолвера.
- **Пограничные случаи:** Внешний контур (живая оплата, прод-БД, сторонний сервис) — фиксируется краснота ДО починки, дефект не создаётся. Эквивалентные мутации (семантически идентичный код) — выжившая мутация не всегда дефект теста.
- **Антипаттерны:** «Проверено мутацией» без указания, какой именно, — незаконченное утверждение. Мутация в выдуманной форме дефекта: доказывает, что гард работает, но не что он ловит то, от чего защищает. Удалить код при выжившей мутации, прочитав её как «код не нужен». Подменять приёмку гарда мутационным **score** по кодовой базе — см. карту противоречий §4.
- **Связи:** дополняет [P-03] (CHK-WIRE) и [P-04] (CHK-ORPHAN) — они говорят, ЧТО проверять, P-13 говорит, как принять саму проверку; смыкается с [P-11] (Code-Change Discipline: evidence перед правкой). Отдельно — `dpf/development.md` → P-11: там mutation **score** как метрика сьюта, здесь mutation **proof** как приёмка одного гарда; предметы разные (A.7).
- **Когда НЕ применять:** TDD-тест новой фичи — RED-фаза и есть его доказательство, повторное требование превращается в ритуал. Прототипы и throw-away код.
- **SoTA-источник:** Полевые данные Core Team (2026-08): **69 уроков ремесла из 175 в двух проектах** касаются гардов и проверок; зафиксированы серии 9 / 6 / 3 ложно-зелёных гардов за одну сессию. Литературная опора — mutation testing как техника (см. `dpf/development.md` P-11); прямого первоисточника, требующего мутационную пробу как **гейт приёмки единичного гарда**, в обзоре не найдено — паттерн выведен из практики.
- **Проверка качества (E.21):** В дифе не остаётся ни одного гарда, для которого нельзя назвать роняющую его мутацию. У каждого перечисляющего гарда есть половина «найдено не ноль». У каждой сверки — утверждение непустоты сторон до равенства. У каждого гарда назван потребитель красноты (CI / хук / гейт).
...
| **Guard Without Proof** | Гард добавлен, краснота ни разу не предъявлена | Может не различать мир с дефектом и без; снимает вопрос и создаёт ложную защищённость | [P-13] — приёмка через мутацию |
| **Vacuous Enumeration** | Перечисляющий гард зелен, потому что перечисление пусто (0 модулей → 0 нарушений) | Устойчив: переживает рефакторы и переезды, ничем себя не выдавая | [P-13] — половина «найдено не ноль» |
| **Empty Comparison** | Сверка печатает «совпадает», когда обе стороны не вычислились | Отказ инструмента совпадает с самым благополучным ответом | [P-13] — непустота прежде равенства |
...
| **Mutation score vs mutation proof** | Инструментальная школа: mutmut/Stryker периодически, целевой score 80%+ по модулю — объективная метрика качества сьюта | Полевая школа: важно не среднее по базе, а предъявленная краснота **конкретного** гарда в момент сдачи | Разные предметы (A.7), не заменяют друг друга: score отвечает «насколько хорош сьют вообще», proof — «этот гард отличает мир с дефектом от мира без». Proof обязателен всегда, score — по возможности |
...
|------|--------------|-------------------|-------------------------------------------------------------|
| 2026-08-25 | Добавлен P-13 (Mutation Proof как приёмка гарда) по полевым данным двух проектов: 69 уроков из 175 про гарды, серии 9/6/3 ложно-зелёных гардов за сессию. Закрыт объявленный пробел «mutation testing упомянут в глоссарии, но нет паттерна» — с разграничением score ⊥ proof (A.7). +3 антипаттерна, +1 строка карты противоречий | draft→draft | Открытое: P-13 выведен из практики — первоисточника, требующего мутационную пробу как гейт приёмки единичного гарда, в обзоре не найдено; при появлении — уточнить провенанс |
