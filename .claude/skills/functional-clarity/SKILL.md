...
See `references/style-guide.md` for the full version with details.
...
## Code-Change Discipline (Principle 2 deepened)

When modifying code you did NOT write in this session — refactor, bugfix, extension, maintenance — apply the **7-step Code-Change Discipline**:

1. **Понять идею** — зачем код написан так, не «как работает», а «какую задачу решает»
2. **Поставить идею под сомнение первой** — кривая идея → меняй идею, верная → меняй реализацию
3. **Определить свои допущения** — каждое допущение есть гипотеза
4. **Проверять гипотезы кодом** — тест, запуск, лог. Prediction → Run → Compare
5. **Спрашивать человека** там где не уверен как система ДОЛЖНА вести себя (поведение vs идея)
6. **Не менять контракт метода** без обсуждения — сигнатура/return/поведение для caller-а = breaking
7. **Не убирать информацию** — шумный лог заменить лучшим, не удалять

Подкрепляющие FPF-guards: A.7 (Strict Distinction — role ≠ implementation), A.10 (Evidence Graph — claim without evidence is opinion), A.11 (Ontological Parsimony — add only what cannot be subtracted), A.1.1 (Bounded Context — meaning is local).

Полная версия: `references/code-change-discipline.md`.

## Reference Documentation

- **`references/principles.md`** — Complete 22 principles with detailed explanations and code examples
- **`references/style-guide.md`** — Full style guide with details on each point
- **`references/code-change-discipline.md`** — 7-step algorithm + FPF-guards для правок существующего кода
- **`references/python.md`** — Python-specific: modern syntax, pathlib, type annotations, pytest patterns
- **`references/bash-scripting.md`** — Bash: версия-aware синтаксис, `#!/usr/bin/env bash`, `set -euo pipefail`, trap-очистка

## Reference Loading Order

1. **SKILL.md** provides the working summary for applying principles during coding
2. IF user asks for detailed explanation of a specific principle → read `references/principles.md`
3. IF writing or reviewing code and need detailed style guidance → read `references/style-guide.md`
4. IF modifying code you did not write in this session → read `references/code-change-discipline.md`
5. IF writing Python code → read `references/python.md`
6. IF writing a bash/shell script → read `references/bash-scripting.md`
