...
# Прежняя форма подсказки. Она ОСТАЁТСЯ в матчере навсегда: у потребителей уже
# заведены решения по старому шаблону, и выкинуть строку значило бы счесть их
# подсказку заполненной — гард замолчал бы ровно там, где обязан краснеть.
D=$(new_dir); add_dec "$D" DEC-009 accepted 2026-08-26 2026-12-01 "Условие, при котором решение отменяется (DRR — FPF decay)" ""
check "порог из прежнего шаблона порогом не считается" 0 "$(run_code "$D")"

# Нынешняя форма: `DRR` в шаблоне заменён на `Evidence Decay` — в FPF распад
# подтверждения зовётся так, а `DRR` это запись обоснования (E.9).
D=$(new_dir); add_dec "$D" DEC-009 accepted 2026-08-26 2026-12-01 "Условие, при котором решение отменяется (FPF Evidence Decay)" ""
check "порог из нынешнего шаблона порогом не считается" 0 "$(run_code "$D")"
...
# --- Решение без шапки: одна причина вместо находки на каждое поле ------------
# Мир с дефектом: у файла без шапки пусты ВСЕ поля, и прибор перечислял его в
# каждой группе отдельно. Замер прогона по боевым проектам (2026-09-04): 22
# решения из 22 в одном проекте дали 44 находки вместо одной причины «форма не
# та», и настоящие находки соседних приборов терялись за этой кучей. Внимание
# конечно — это гейт 9, применённый к самому прибору.
headerless_dec() {  # $1=каталог $2=имя
  printf '# %s — решение\n\n- **Дата:** 2026-06-15\n- **Статус:** Accepted\n\n## Решение\nтекст\n' \
    "$2" > "${1}/${2}.md"
}

D=$(new_dir); headerless_dec "$D" DEC-001
check "решение без шапки — находка" 1 "$(run_code "$D")"
check "…и названа одна причина" "да" "$(says "$(run_out "$D")" "РЕШЕНИЕ БЕЗ ШАПКИ (1)")"
check "…и названа форма" "да" "$(says "$(run_out "$D")" "adr-template.md")"
check "…и не перечислено по полям" "нет" "$(says "$(run_out "$D")" "НЕТ ДАТЫ ПРИНЯТИЯ")"

# Смешанная форма: бесшапочные уходят в свою группу, остальные проверяются как были.
D=$(new_dir); headerless_dec "$D" DEC-001
add_dec "$D" DEC-002 accepted "$TODAY" "" "" ""
check "смешанная форма — обе группы" 1 "$(run_code "$D")"
check "…бесшапочное названо один раз" "да" "$(says "$(run_out "$D")" "РЕШЕНИЕ БЕЗ ШАПКИ (1)")"
check "…и шапочное разобрано по полю" "да" "$(says "$(run_out "$D")" "НЕТ ДАТЫ ПРОВЕРКИ (1)")"
