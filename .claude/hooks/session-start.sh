...
#
# ТИР: показ — печатает находки, ничего не требует
...
assets_expiry() {
  ...
  # Берём ТОЛЬКО находки: поток отказов (stderr) плюс единственную предупреждающую
  # строку из отчёта. Прежняя версия сливала оба потока целиком и печатала блок
  # «Активы» даже на пустом реестре — прибор-паникёр, которого перестают читать.
  # `2>&1 >/dev/null` — тот же приём, что у объёма памяти: fd2 уходит в захват,
  # fd1 в никуда. Отчёт о разборе не берётся, «Записей ноль» тоже: это не находка.
  findings=$(bash "${checker}" "${file}" 2>&1 >/dev/null || true)
  soon=$(bash "${checker}" "${file}" 2>/dev/null | grep -E '^Срок наступает' || true)
  findings=$(printf '%s\n%s' "${soon}" "${findings}" | sed '/^[[:space:]]*$/d')
  [ -n "${findings}" ] || return 0
...
# --- Обновление поставки ----------------------------------------------------
# Замер (аудит по этапам жизни, 2026-09-14): 11 установок из 17 на версиях не выше 5.0,
# ни одна — на текущей; копия не знала, что есть версия новее. Содержание проверки
# живёт в upgrade-notice.sh — здесь только показ; сеть, кэш и таймаут — там же.
# Печатается ПЕРВЫМ: это единственная находка, которая касается самого инструмента.
upgrade_notice() {
  local checker
  checker="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)/upgrade-notice.sh"
  [ -f "${checker}" ] || return 0
  bash "${checker}" . 2>/dev/null || true
  return 0
}

# --- Порядок вывода: сначала ПЕРЕМЕННОЕ, потом ПОСТОЯННОЕ ------------------
# Протокол ниже одинаков в каждой сессии — восемь блоков неизменного текста.
# Находки меняются, и именно они требуют действия. Замер, ради которого порядок
# перевёрнут: на боевом проекте хук печатал 61 строку и 14 блоков, из которых
# протокол занимал первые восемь, а находки про ЭТОТ проект шли последними.
#
# Внутри памяти ролей объём идёт ПЕРЕД возрастом: возрастной термометр на боевых
# данных печатал пять наименее нагруженных ролей (brand 28 дн., devops 182 дн.),
# а `dev` с 988 КБ — нечитаемой памятью — уходил ниже, в блок объёма.
upgrade_notice
meaning_ladder
memory_volume
memory_health
