...
#   3) ИСТОЧНИК БЕЗ АДРЕСА ПРОВЕРКИ — `enforced_by` пуст и долг не объявлен;
#   4) ДОЛГ ПРОСРОЧЕН — «принято как долг до <дата>», дата прошла.
#
# ТРЕТИЙ ВИД КРАСНОГО — клапан, а не лазейка. Красное бывает трёх видов: снимается
# прогоном (адрес проверки) · требует правки · **принято как долг человеком, с датой**.
# Закрыть запись без адреса проверки можно — объявив в колонке «Как закрыто»:
# «принято как долг до 2026-10-01, владелец cto». Пока дата не прошла, прогон
# зелёный и долг перечислен явно. Дата прошла — красное вернулось. Долг без даты
# долгом не является: это забывание с красивым именем.
#
...
while IFS= read -r row; do
  ...
  howclosed=$(trim "$(printf '%s' "${row}" | cut -d'|' -f6)")
  ...
  case "${value}" in
    'НЕТ_ФАЙЛА') dead_source+=("${id} → ${dec}"); continue ;;
    ''|'-'|'—'|\<*|*\>) ;;
    *) continue ;;
  esac

  # Адреса нет — принимаем объявленный долг с датой. Без даты долг не считается.
  debt_until=$(printf '%s' "${howclosed}" | grep -oE 'долг[^0-9]{0,20}[0-9]{4}-[0-9]{2}-[0-9]{2}' | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}' | head -1)
  if [ -z "${debt_until}" ]; then
    unenforced+=("${id} → ${dec}")
  elif [[ "${debt_until}" < "${TODAY}" ]]; then
    expired_debt+=("${id} → ${dec} (долг до ${debt_until})")
  else
    live_debt+=("${id} → ${dec} (до ${debt_until})")
  fi
...
if [ ${#expired_debt[@]} -gt 0 ]; then
  ...
  printf 'Дата, до которой долг был принят, прошла — красное вернулось. Одно из двух:\n' >&2
  printf 'заполнить `enforced_by` у решения либо продлить долг новой датой и причиной.\n' >&2
  printf 'Продление без причины — то же забывание, только с обновлённым числом.\n' >&2
  failed=1
fi
...
if [ ${#unenforced[@]} -gt 0 ]; then
  ...
  printf 'Либо объяви долг в колонке «Как закрыто»: «принято как долг до 2026-10-01,\n' >&2
  printf 'владелец <роль>». Долг с датой законен; долг без даты — забывание.\n' >&2
  failed=1
fi
