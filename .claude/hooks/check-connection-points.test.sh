...
# --- Слой, которого у проекта быть не может ----------------------------------
# Мир с дефектом: канон точек писался от разработки — `craft:tdd`, `craft:stacks`,
# `module-contracts` про код. Проект-исследование или организационный обязан был
# держать эти строки и объявлять деградацию по Red-Green-Refactor, которого у него
# не будет никогда; удалить строку он не мог — прибор требовал каждую точку канона.
# Замер: проект про переезд офиса дал три красных за отсутствие точек, которых у
# него быть не может.
P=$(make_points "craft:tdd|неприменимо|проект не пишет код: работа идёт документами")
check "слой назван неприменимым — законно" 0 "$(run_code "$P" "" "$LIST_OK")"

P=$(make_points "craft:tdd|неприменимо|—")
check "неприменимость без причины — находка" 1 "$(run_code "$P" "" "$LIST_OK")"
check "…и сказано, чего не хватает" "да" \
  "$(says "$(run_out "$P" "" "$LIST_OK")" "названа неприменимой, а почему — не сказано")"

# Граница: «неприменимо» закрывает точку, а не отменяет проверку установки.
