...
# --- Разобрать нечем ---------------------------------------------------------
# `sed -i ''` — форма BSD: на GNU пустая строка становится СКРИПТОМ, правка молча
# не происходит, и случай проходит по неверной причине. Переписываем переносимо.
R=$(make_registry 20 3 1)
sed 's/^## Открытые$/## Всякое/' "$R" > "$R.tmp" && mv "$R.tmp" "$R"
