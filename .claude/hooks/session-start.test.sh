...
# 2. МИР С ДЕФЕКТОМ: память роли старше порога — прибор обязан её назвать.
R=$(make_roles); add_role "$R" keeper "$(days_ago 30)"
OUT=$(run_hook "$R")
check "отставшая роль названа" "да" "$(says "$OUT" "keeper")"

# 3. МИР С ДЕФЕКТОМ: памяти нет вовсе — отдельный диагноз, не «отстаёт».
R=$(make_roles); add_role "$R" guardian -
OUT=$(run_hook "$R")
check "пустая память названа" "да" "$(says "$OUT" "Памяти нет вовсе: guardian")"

# 4. Ложное срабатывание: свежая память молчит. Прибор-паникёр не читают.
...
# 5. Разделение диагнозов: отставшая и пустая роли не сливаются в один список.
...
# 6. Безвредность: каталога ролей нет (проект ещё не развёрнут) — хук не падает
