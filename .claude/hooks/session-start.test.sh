...
# --- Точки подключения: слой заявлен закрытым, а инструмента нет --------------
# Мир с дефектом: таблица говорит «закрыто плагином», плагин не установлен или
# выключен. Роль зовёт навык, которого в сессии нет, и не падает — она работает
# без ремесла молча. Узнать об этом в `/end-session` значит узнать после работы.
...
# Список плагинов подменяем: настоящий CLI в наборе не зовём — он медленный и
# зависит от машины, а проверяем мы показ находок, а не сам прибор.
empty_list() {
  local d f; d=$(mktemp -d); TRASH+=("$d"); f="$d/list.sh"
  printf '#!/bin/bash\nprintf "Installed plugins:\\n\\n"\n' > "$f"; chmod +x "$f"; printf '%s' "$f"
}
run_with_points() {
  CONNECTION_POINTS_FILE="$1" CONNECTION_POINTS_CANON="/nonexistent-canon-$$" \
    PLUGIN_LIST_CMD="$2" ROLES_DIR="/nonexistent-$$" bash "$HOOK" 2>/dev/null
}

L=$(empty_list)
P=$(make_points '`smotrovaya`' 'ручная проверка')
OUT=$(run_with_points "$P" "$L")
check "заявленный плагин не установлен — названо" "да" "$(says "$OUT" "НЕ УСТАНОВЛЕН")"
check "…под своим заголовком" "да" "$(says "$OUT" "**Точки подключения.**")"

# Незакрытая точка с объявленной деградацией — законное состояние, молчим.
P=$(make_points '—' 'ручная проверка, покрытие не считается')
check "объявленная деградация молчит" "нет" \
  "$(says "$(run_with_points "$P" "$L")" "**Точки подключения.**")"

# Дыра без имени — находка даже без списка плагинов.
P=$(make_points '—' '—')
check "дыра без объявленной деградации названа" "да" \
  "$(says "$(run_with_points "$P" "$L")" "**Точки подключения.**")"

# Безвредность и цена: файла точек нет — блока нет, и CLI не зовётся вовсе.
OUT=$(run_with_points "/nonexistent-points-$$" "/nonexistent-cli-$$"); code=$?
check "файла точек нет — тихо и без падения" "0/нет" "$code/$(says "$OUT" "**Точки подключения.**")"
