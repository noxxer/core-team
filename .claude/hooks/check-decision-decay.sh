...
#
# ТИР: счёт — решение устарело; разбирается пачкой, а не по одному
#
...
#   · 85 не имеют `review_due` вовсе — механизма распада (FPF Evidence Decay) у них нет,
...
#
# Одна работа над одним предметом «решение не устарело молча». Три находки —
...
#   exit 1 — хоть одно устарело, либо разобрать каталог не удалось
#
...
# Разбор вынесен в библиотеку: YAML-комментарий читался как часть значения, а
# шаблоны фреймворка комментарии содержат обильно. Замер прогона: решение из
# штатного шаблона дало две ложных находки — пустое `accepted_until` сочлось
# заполненным, заполненное `metric_for_revisit` — пустым.
...
if [ -f "${_lib_fm}" ]; then
  ...
  # Сломанная библиотека проверку не роняет: работаем прежним путём.
...
is_placeholder() {  # $1=значение
  case "$1" in
    ''|'-'|'—'|'TBD'|'tbd') return 0 ;;
    \<*\>|\[*\]) return 0 ;;
    *'Условие, при котором'*|*'DRR — FPF decay'*|*'FPF Evidence Decay'*|*'обязательно если'*) return 0 ;;
    *) return 1 ;;
  esac
}
...
# Половина «найдено не ноль» стоит на РАЗБОРЕ, а не на количестве: ноль решений
# у только что развёрнутого проекта — законное состояние, а не отказ прибора.
# Замер: свежий проект из шаблонов краснел на первом же `/end-session` по причине,
# к которой пользователь не имеет отношения.
if [ "${examined}" -eq 0 ]; then
  printf 'Решений ноль — либо их не фиксировали, либо проект только развёрнут.\n'
  printf 'Первое же архитектурное решение обязано попасть в `project/decisions/`.\n'
  exit 0
fi
...
if [ ${#no_review[@]} -gt 0 ]; then
  ...
  printf 'Решение без `review_due` действует вечно: механизма распада (FPF Evidence Decay) у него нет.\n' >&2
