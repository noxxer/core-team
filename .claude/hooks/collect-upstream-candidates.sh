...
#
# Корзина `stacks/<стек>` ведёт в навык `core-team-dev:stacks` (содержание уехало
# в плагин ремесла), корзина `ядро` — в `.claude/`. Имя корзины не меняется:
# она называет ПРЕДМЕТ находки, а не каталог, где он сейчас лежит.
#
