...
2. `project/roles/<role>/context.md` — **копированием шаблона** `.claude/templates/project/role-context-template.md`; `Write` по этому пути закрыт `permissions.deny` намеренно;
...
Для `hybrid` активны все одиннадцать: удалять нечего, и это **законный исход, а не пропуск**.
