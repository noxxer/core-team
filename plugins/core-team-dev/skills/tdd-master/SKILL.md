...
### Три лица ложно-зелёной проверки

> **Ложно-зелёная** — зелёная не потому, что дефекта нет, а потому что проверка его не смотрит.
> В уроках проектов этот класс называется «вакуозный гард».

`assert True` виден сразу. Опасны те, что выглядят рабочими: тест зелёный по трём разным причинам,
и все три снаружи одинаковы.

| Лицо | Что происходит | Измеренный пример | Лечение |
|---|---|---|---|
| **Вхолостую** | входа, на котором проверка падает, не существует | гард обходил **0 модулей**: «0 нарушений → зелено» | половина «найдено не ноль» рядом, в том же файле |
| **Мимо цели** | вход есть, но не тот | доменный гард искал литерал `example.com`, а вызовы написаны `import` — литералов в коде нет вовсе | привязаться к вещи, а не к её имени/метке/пути |
| **Не подключено** | проверка падает верно, её падение никто не потребляет | красный `pytest` не останавливал выкатку в `cd.yml` | назвать потребителя красноты |

**Диагностический вопрос:** на каком входе эта проверка падает — и тот ли это вход, от которого она
защищает? Ответ «ни на каком» или «не знаю» означает, что гарда нет.

**Приёмка.** Не видел свой тест красным — докажи мутацией: назови мутацию, подтверди дифом, что она
попала, приложи красный и зелёный прогоны (шаг **Prove** в `knowledge/core-protocols.md`).
Выжившая мутация — не «код не нужен», а «тест проходит другим путём».

**Подозрительный признак:** тест сравнивает результат функции с результатом другой своей же функции
вместо внешнего факта; проверяет вхождение подстроки вместо строки целиком; сравнивает число
элементов вместо состава.
...
| Neither detected | Use TDD_GUIDE only | `references/tdd-guide.md` |
...
```
1. ALWAYS read: references/tdd-guide.md (core methodology)
2. ALWAYS read: references/p0-defaults.md (context-adaptive defaults)
...
- **`references/tdd-guide.md`** - Core TDD methodology (Kent Beck, Uncle Bob, FIRST principles, anti-patterns)
- **`references/p0-defaults.md`** - Context-adaptive default values (FK=minimal, entry=maximal, edge=explicit)
