...
- **User-home paths** — `/Users/<name>/...`, `/home/<name>/...`, `C:\\Users\\<name>\\...` → replace the user segment with `<user>` and keep the relative tail intact (e.g. `/Users/alice/projects/foo` becomes `/Users/<user>/projects/foo`). Project structure is signal worth preserving; the operator's username is not.
